...
## Finalize the current Display LOD0 checkpoint

The current offline checkpoint combines the seven-draw Helmet head/hair model,
the eight-draw Outfit body/garment model, and 20 generated runtime swatches.
It remains a Display-only LOD0 package; `_Driver` assets and native LOD1-LOD3
geometry are not included yet.

Build the material candidates and their explicit MatI profiles:

```powershell
python .\scripts\build_si_display_material_pipeline.py
```

The materialized candidates must be verified against the pre-material geometry
candidate. This mode requires every non-MatI blob to stay identical, performs a
lossless outer-bundle round trip, and performs a lossless nested round trip plus
exact profile check for every MatI:

```powershell
python .\scripts\verify_fh6_modelbin_candidate.py `
  .\work\si\fbx-source\milestone-05-modelbin-lod0-v002\helmet\Helmet_Race_Modern.lod0-head7.modelbin `
  .\work\si\fbx-source\material-pipeline-v002\candidates\Helmet_Race_Modern.lod0-materials-v002.modelbin `
  .\work\si\fbx-source\milestone-04-intermediate-v002\lod0\head-hair.intermediate.manifest.json `
  --component helmet `
  --material-profile .\work\si\fbx-source\material-pipeline-v002\profiles\helmet-final-lod0-v002.json `
  --report .\work\si\fbx-source\milestone-06-display-package-lod0-v001\verification\helmet.final.structure-material.report.json
```

Run the equivalent command with the Outfit geometry candidate, body/garment
manifest, and Outfit material profile. Then generate decoded-pixel previews:

```powershell
python .\scripts\build_si_display_material_preview.py
```

Before archive assembly, place a clean retail `characters.zip` at
`work/si/components/baselines/textures-characters/characters.original.zip`.
The package builder requires SHA-256
`b54507011707db4f4037d8f63d54519fd9d8d45ea527c3cf93ea36ad1d16e77e`,
1990 entries, valid CRCs, and no existing `si_` or `jsspsi` entries.

Build the three copied archives without touching the game directory:

```powershell
python .\scripts\build_si_display_lod0_package.py
python .\scripts\deploy_si_display_package.py `
  --report .\work\si\fbx-source\milestone-06-display-package-lod0-v001\deployment\dry-run-v001.json
```

The deploy command is dry-run by default. It verifies the candidate hashes,
ZIP CRCs, all 4096-byte payload alignment fields, and the three exact game
targets. Only an explicitly authorized run may add `--apply`. Restore a prior
apply using its completed deployment report:

```powershell
python .\scripts\deploy_si_display_package.py `
  --restore <deployment-report.json> `
